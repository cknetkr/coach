window.MATH_S5 = String.raw`<div id="math-s5" class="inner-panel" style="display:none;">
    <h2 class="panel-title">🧠 공식 카드 — 1타 강사 압축 정리</h2>
    <p class="panel-desc">공식을 외우는 단계가 아니라, 이 공식이 어떤 문제에서 왜 쓰이는지까지 함께 떠오르게 만드는 최종 정리 페이지입니다.</p>

    <section class="math-stage-card hero">
      <div class="math-stage-card-head">
        <div class="math-stage-card-kicker">STEP 5 · MEMORY TO UNDERSTANDING</div>
        <div class="math-stage-card-title">공식 카드는 식을 외우는 칸이 아니라, 문제를 보자마자 어떤 개념을 꺼낼지 연결하는 훈련칸이어야 합니다.</div>
        <div class="math-stage-card-desc">카드를 펼칠 때마다 <strong>정의, 쓰는 조건, 자주 틀리는 반례, 연결 문제</strong>를 같이 떠올리십시오. 그래야 시험 직전에도 공식을 실제 풀이 도구로 쓸 수 있습니다.</div>
      </div>
      <div class="math-stage-note">권장 공부량: <strong>하루 3회독</strong>, <strong>핵심 카드 24장</strong>, <strong>취약 카드 8장 재복습</strong>을 기본 루틴으로 잡으십시오.</div>
    </section>
    ...
    <div class="mc">
      ...
      <div class="btn-row" style="margin-top:1rem;">
        <button class="btn-secondary" onclick="document.querySelectorAll('#math-s5 .formula-cards .formula-card').forEach(c=>c.classList.remove('revealed'))">🔄 다시 가리기</button>
        <button class="btn-secondary" onclick="document.querySelectorAll('#math-s5 .formula-cards .formula-card').forEach(c=>c.classList.add('revealed'))">👁 전체 펼치기</button>
      </div>
    </div>
...
eq1, b>0$
      ...
      <div class="btn-row" style="margin-top:1rem;">
        <button class="btn-secondary" onclick="document.querySelectorAll('#s5-graph .formula-card').forEach(c=>c.classList.remove('revealed'))">🔄 다시 가리기</button>
        <button class="btn-secondary" onclick="document.querySelectorAll('#s5-graph .formula-card').forEach(c=>c.classList.add('revealed'))">👁 전체 펼치기</button>
      </div>
    </div>
